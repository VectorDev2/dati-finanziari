...
def compute_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 7. Price Position (FORZIAMO .values per avere array 1-D)
    price_pos = (df['Close'] - df['BB_lower']) / (df['BB_upper'] - df['BB_lower'])
    df['Price_Position'] = price_pos.values.astype(float)

    return df

def predict_with_model(ticker: str, start_date="2020-01-01", end_date=None):
    # scarica aggiustando per split/adj
    df = yf.download(ticker, start=start_date, end=end_date, auto_adjust=True)
    if len(df) < 60:
        raise ValueError("Troppi pochi dati per le feature tecniche.")

    df = compute_technical_indicators(df).dropna()
    df = df[df['Return'].abs() > 0.002]  # filtra rumore
